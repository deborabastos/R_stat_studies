...
#Forma o conjunto de dados historico contendo vinte e nove clientes selecionados aleatoriamente do 
#banco de dados e a armazena em um data frame chamado dados
...
### Teste Qui-quadrado
# H0: As variáveis são independentes (não tem correlaçào)-> A variável estado civil não tem relação com o fato de possuir imóvel próprio
# H1: As variáveis são dependentes (tem correlaçào) -> A variável estado civil tem relação com o fato de possuir imóvel próprio

# p-valor > alpha (alto) aceita H0
# p-valor < alpha (baixo) rejeita H0

# Neste caso, p-valor = 0,72 (alto) -> Não há evidência estastíticas para rejeitar H0, portanto, 
# não se pode afirmar que as variáveis estado civil e possuir imóvel próprio tenham correlação
...
### Teste t 
# H0:A média das variáveis são estatisticamente iguais > A média de consumo é igual idenpendente de ter ou não imóvel?
# H1:A média das variáveis são estatisticamente diferentes -> A média de consumo é diferente idenpendente de ter ou não imóvel?

# p-valor > alpha (alto) aceita H0
# p-valor < alpha (baixo) rejeita H0

# Neste caso, ao p valor de 72,26%, não há evidências para rejeitar a hipótese nula de igualdade de médias, 
# ou seja, a média do consumo de quem tem imóvel próprio não é estatisticamente diferente da média de consumo 
...
#Se p-valor do teste F < alpha, rejeita H0, então, o modelo é válido
